...
async function renderPlayVideoData(videoId){
    try{
        ...
        views.innerHTML = calcViews(videoStats.statistics.viewCount) + ' Views | ';
        views.innerHTML += calcTime(videoStats.snippet.publishedAt) + ' ago';
        likes.innerHTML += calcViews(videoStats.statistics.likeCount);
        commentCount.innerHTML = calcViews(videoStats.statistics.commentCount) + ' Comments';
        ...
        subscribers.innerHTML = calcViews(channelStats.items[0].statistics.subscriberCount) + ' Subscribers';
...
async function getVideoResults(searchString){
    try{
        ...
        addDataOnListSection(result.items);
    }
    catch(error){
...
async function addDataOnListSection(videos){
    try{
        videos.forEach(async (video) => {
            ...
            <div class="videoCard" id="videoCard" value="${video.id.videoId}">
            <div class="listVideoThumbnail">
                ...
                <div class="listVideoDuration" id="listVideoDuration">${calcDuration(videoStats.contentDetails.duration)}</div>
            </div>
            <div class="listVideoDetails">
                ...
                <div class="listVideoInfo"><span id="listViews">${calcViews(videoStats.statistics.viewCount)} Views</span> | <span id="listPubTime"> ${calcTime(video.snippet.publishTime)}  ago</span></div>
            </div>
            </div>
            `
        });
